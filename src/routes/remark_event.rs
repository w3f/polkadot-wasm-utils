...
impl Component for RemarkComponent {
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            Message::OnlineClientCreated(online_client) => {
                ...
                self.set_message("I want Billcoins".into());
            }
            ...
            Message::SignWithAccount(i) => {
                if let SigningStage::SelectAccount(accounts) = &self.stage {
                    ...
                    let remark_call = polkadot::tx()
                        .system()
                        .remark_with_event(self.message.as_bytes().to_vec());
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let stage_html: Html = match &self.stage {
            ...
            SigningStage::EnterMessage => {
                ...
                html!(
                    <>
                        <div class="mb"><b>{"Replace the text below with your custom remark message if needed:"}</b></div>
        ...
        html! {
            <div>
                <h1>{"Submit Remark with Event"}</h1>
                {message_html}
                {signer_account_html}
                {stage_html}
            </div>
