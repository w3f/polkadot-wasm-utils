...
impl Component for VestingComponent {
        ...
        match msg {
            ...
            Message::SignWithAccount(i) => {
                if let SigningStage::SelectAccount(accounts) = &self.stage {
                    ...
                    let target_account_id: AccountId32 = self.message.parse().unwrap();
                    ...
                    let vesting_call = polkadot::tx()
                        .vesting()
                        .vest_other(subxt::utils::MultiAddress::Id(target_account_id.clone()));
                    ...
                    ctx.link().send_future(async move {
                        ...
                        let Ok(call_data) = api.tx().call_data(&vesting_call) else {
                        ...
                        let Ok(partial_signed) =
                            api.tx().create_partial_signed_offline(&vesting_call, params)
                        else {
