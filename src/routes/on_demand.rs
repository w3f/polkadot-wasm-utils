...
use crate::services::{extension_signature_for_extrinsic, get_accounts, paseo, Account};
...
impl Component for OnDemandComponent {
    type Message = Message;

    type Properties = ();

    fn create(ctx: &Context<Self>) -> Self {
        ctx.link().send_future(OnlineClient::<PolkadotConfig>::from_url("wss://paseo.rpc.amforc.com").map(|res| {
            match res {
                Ok(online_client) => Message::OnlineClientCreated(online_client),
                Err(err) => Message::Error(anyhow!("Online Client to Paseo Network could not be created:\n{err}")),
            }
        }));
        OnDemandComponent {
            message: "".to_string(),
        ...
        match msg {
            ...
            Message::SignWithAccount(i) => {
                if let SigningStage::SelectAccount(accounts) = &self.stage {
                    ...
                    let max_amount: u128 = 1000000000000u128;
                    let para_id: u32 = self.message.parse().unwrap();

                    self.stage = SigningStage::Signing(account.clone());
                    //let target: subxt::utils::MultiAddress<subxt::utils::AccountId32, ()> = MultiAddress::Index(());

                    let on_demand_call = paseo::tx()
                        .on_demand()
                        .place_order_allow_death(max_amount, primitives::Id(para_id));
                    ...
                    ctx.link().send_future(async move {
                        ...
                        let Ok(call_data) = api.tx().call_data(&on_demand_call) else {
                        ...
                        let Ok(partial_signed) =
                            api.tx().create_partial_signed_offline(&on_demand_call, params)
                        else {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let stage_html: Html = match &self.stage {
            ...
            SigningStage::EnterMessage => {
                ...
                html!(
                    <>
                        <div class="mb"><b>{"Replace the \"ParaID\" below with your on-demand chain's ID:"}</b></div>
            ...
            SigningStage::SelectAccount(accounts) => {
                if accounts.is_empty() {
                    html!(<div>{"No Web3 extension accounts found. Install Talisman, SubWallet or the Polkadot.js extension and add an account."}</div>)
                } else {
                    html!(
                        <>
...
async fn submit_wait_finalized_and_get_extrinsic_success_event(
...
) -> Result<paseo::system::events::ExtrinsicSuccess, anyhow::Error> {
    ...
    for event in events.find::<paseo::system::events::ExtrinsicSuccess>() {
        web_sys::console::log_1(&format!("{:?}", event).into());
    }

    let success = events.find_first::<paseo::system::events::ExtrinsicSuccess>()?;
